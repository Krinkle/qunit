...
export default class HtmlReporter {
   ...
   * If set to undefined, then at `QUnit.begin()` we look for `#qunit`
   * and use that if it exists.
   * If set to null, then no report will be created.
   */
  constructor (QUnit, options) {
    // Don't init the HTML Reporter in non-browser environments
    if (!window || !document || options.element === null) {
      return;
    }
    this.stats = {
      failedTests: [],
  ...
  onRunStart (runStart) {
    if (!this.element) {
      this.element = DOM.id('qunit') || null;
    }
